...
### v0.6.0 — Provider Expansion (~2.5h)

Target: Add new LLM providers following the existing integration pattern (Provider enum → endpoint → models → PROVIDERS → settings → i18n).

| Feature                           | Effort    | Details                                                                                                                                          |
| --------------------------------- | --------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| Mistral AI provider               | ~1.5h     | Backend: Provider enum variant, endpoint, models, categorization. Frontend: PROVIDERS entry, settings API key input, i18n provider name          |
| Custom OpenAI-compatible provider | ~1h       | Backend: "custom" endpoint reading URL from DB, Custom Provider variant. Frontend: editable URL + API key fields in settings, i18n provider name |
| **Total**                         | **~2.5h** | 4 source file edits + model list + 10 i18n keys                                                                                                  |

**No changes needed**: `stream_chat` is already OpenAI-compatible. `ChatInput` and `ProviderSelector` render dynamically from `PROVIDERS`.

### v0.7.0 — Chat UX Enhancements (~6.5h)

Target: Frontend-heavy features improving daily chat interaction quality.

| Feature                           | Effort    | Details                                                                                                                                     |
| --------------------------------- | --------- | ------------------------------------------------------------------------------------------------------------------------------------------- |
| System notification on completion | ~30min    | `tauri-plugin-notification` + `send_completion_notification` command fired on stream complete. Native OS notification with response preview |
| Prompt library                    | ~3h       | New `prompts` SQLite table + Rust CRUD commands + `PromptPicker.svelte` component + `/prompt` command                                       |
| Image paste/drag                  | ~3h       | Accept `image/*` in ChatInput, base64 encode, include as multimodal `image_url` content blocks in API request. Show preview thumbnails      |
| **Total**                         | **~6.5h** | 3 new modules (Rust + Svelte + service) + existing API extension + capability config                                                        |

### v0.8.0 — Import & Branching (~9h)

Target: Data management features for bringing conversations in and exploring alternatives without losing history.

| Feature                | Effort  | Details                                                                                                                                                          |
| ---------------------- | ------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Chat import            | ~4h     | New `import.rs` Rust module parsing ChatGPT JSON, Claude JSON, and markdown exports. File picker + format selector in Settings UI. Inserts as new sessions       |
| Conversation branching | ~5h     | Migration adding `parent_message_id` to messages. `fork_session` command cloning session at branch point. GitFork icon on assistant messages + sidebar tree view |
| **Total**              | **~9h** | 2 new Rust modules + 2 migrations + frontend components + store extensions                                                                                       |

### v0.9.0 — Agent Mode (~10h)

Target: AI autonomy with user confirmation gating.

| Feature               | Effort   | Details                                                                                                                                                                                                            |
| --------------------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Agent mode / tool use | ~10h     | New `agent.rs` Rust module: agent loop → OpenAI function-calling → parse tool calls → execute (read_file, write_file, run_command) → loop until complete. `AgentMode.svelte` panel showing tool calls in real-time |
| **Total**             | **~10h** | 1 new Rust module + skills.rs extension + api.rs tools extension + Svelte component + chat page toggle                                                                                                             |
...
**Baseline (v0.5.0)**: 84 inline tests in 6 files, 0 integration tests. ~12% coverage.

### P1 — DB Integration (Done)

| File          | New Tests | Notes                                                         |
| ------------- | --------- | ------------------------------------------------------------- |
| `sessions.rs` | 20        | CRUD, pin/archive, cascade, ordering, soft-delete, edge cases |
...
| 4    | `skills.rs`        | 14    | Done   | HTTP mocking: get_skill_detail, install_skill, check_updates |
| 4    | `model_cache.rs`   | 1     | Done   | HTTP mocking: refresh_models with mockito                    |
| 4    | `api.rs`           | 3     | Done   | Decoupled `get_api_key_for_test` via AppHandle<MockRuntime>  |

**Target**: ~190 tests across all 16 files (~40-50% coverage). Waves 1-4 add ~101 new tests.

**Waves 1-4 completed** (101 new integration/unit tests added in P2, plus 24 in P1). Total: 185 Rust tests, all passing green!
