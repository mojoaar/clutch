...
export function isAllowedFileType(file: File): boolean {
  ...
  if (imageTypes.some((t) => file.type.startsWith(t.replace('*', '')))) return true;
  if (textTypes.some((t) => file.type.startsWith(t.replace('*', '')))) return true;
...
export function filterFiles(files: FileList | File[]): File[] {
  return Array.from(files).filter((f) => f.size <= 10 * 1024 * 1024 && isAllowedFileType(f));
}
