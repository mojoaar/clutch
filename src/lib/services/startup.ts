...
export async function setStartupConfig(config: StartupConfig): Promise<void> {
  await invoke('set_setting', { key: 'start_on_boot', value: String(config.startOnBoot) });
  await invoke('set_setting', { key: 'start_minimized', value: String(config.startMinimized) });
  await invoke('set_setting', { key: 'close_to_tray', value: String(config.closeToTray) });
}
