...
describe("errorMessage", () => {
  ...
  it("formats a stream interrupted error", () => {
    ...
    expect(errorMessage(err)).toBe(
      "Response stream interrupted for session abc-123",
    );
  });
});
...
describe("isRetryable", () => {
  ...
  it("returns true for timeout errors", () => {
    expect(isRetryable({ kind: "timeout", operation: "fetch" })).toBe(true);
  });
...
describe("retryWithBackoff", () => {
  ...
  it("retries on retryable errors and succeeds", async () => {
    ...
    const result = await retryWithBackoff(fn, () => true, 3, 10);
  ...
  it("throws after max retries", async () => {
    ...
    await expect(retryWithBackoff(fn, () => true, 2, 10)).rejects.toThrow(
      "persistent failure",
    );
  ...
  it("throws immediately on non-retryable errors", async () => {
    ...
    await expect(retryWithBackoff(fn, () => false, 3, 10)).rejects.toThrow(
      "bad input",
    );
    expect(fn).toHaveBeenCalledTimes(1);
  });
