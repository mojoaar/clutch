...
describe("streamChat", () => {
  ...
  it("sets streaming status and initializes assistant message", async () => {
    ...
    expect((chatStore.addMessage as any).mock.calls[0][0].role).toBe(
      "assistant",
    );
    expect((chatStore.addMessage as any).mock.calls[0][0].sessionId).toBe(
      "session-1",
    );
  });
  ...
  it("accumulates text tokens into message content", async () => {
    ...
    expect(chatStore.appendToMessage).toHaveBeenNthCalledWith(
      1,
      assistantId,
      "Hello",
    );
  ...
  it("handles __STREAM_INTERRUPTED__ and sets interrupted status", async () => {
    ...
    const calls = (chatStore.setStreamingStatus as any).mock.calls.map(
      (c: any) => c[0],
    );
  ...
  it("appends error message content on invoke rejection", async () => {
    ...
    const appendCalls = (chatStore.appendToMessage as any).mock.calls.map(
      (c: any) => c[1],
    );
    const errorCall = appendCalls.find((c: string) =>
      c.includes("Network failure"),
    );
...
const _rpWasCalledAtImport =
  (registerQueueProcessor as any).mock.calls.length > 0;
