...
const REQUIRED_NAMESPACES = [
  'chat', 'errors', 'providers', 'startup', 'shortcuts', 'workspaces',
  'skills', 'theme', 'exportSettings', 'settingsTabs', 'settingsSections',
  'profile', 'permissions', 'shortcutActions', 'welcome', 'networkStatus',
  'modelCache', 'dropZone', 'contextBar', 'exportFormats', 'errorBoundary',
  'toast', 'aria', 'about', 'updates',
];
...
describe('i18n locale coverage', () => {
  ...
  it('all locales have the same top-level namespaces as en', () => {
    ...
    for (const [code, locale] of Object.entries(LOCALES)) {
      ...
      if (extra.length > 0) {
        console.warn(`${code}: has extra namespaces not in en: ${extra.join(', ')}`);
      }
    }
  });
